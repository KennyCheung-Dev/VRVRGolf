...
public class OVRPlayerController : MonoBehaviour
{
	...
	public virtual void UpdateMovement()
	{
		...
		if (EnableRotation)
		{
            ...
            if(!canRotate && !OVRInput.Get(OVRInput.Button.SecondaryThumbstick) && !startedCoroutine)
            {
                StartCoroutine(DelayRot());
            }

            if (!OVRInput.Get(OVRInput.Button.SecondaryThumbstick) && canRotate) {
                if (SnapRotation)
                {
