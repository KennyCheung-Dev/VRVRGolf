...
public class OVRScreenFade : MonoBehaviour
{
    ...
    private TeleportScript tlScript;
	...
	void Awake()
	{
        ...
        tlScript = transform.root.gameObject.GetComponent<TeleportScript>();
	}
    ...
    public void fadeInOut() {
        StartCoroutine(FadeIO(0, 1));
    }

    public void toNextLevel(string s) {
        StartCoroutine(goToNextScene(0, 1, s));
    }
    ...
    IEnumerator FadeIO(float startAlpha, float endAlpha) {

        tlScript.isTeleporting = true;
        float elapsedTime = 0.0f;
        while (elapsedTime < fadeTime) {
            elapsedTime += Time.deltaTime;
            currentAlpha = Mathf.Lerp(startAlpha, endAlpha, Mathf.Clamp01(elapsedTime / fadeTime));
        ...
        while (elapsedTime < fadeTime) {
            elapsedTime += Time.deltaTime;
            currentAlpha = Mathf.Lerp(endAlpha, startAlpha, Mathf.Clamp01(elapsedTime / fadeTime));
            SetMaterialAlpha();
            yield return new WaitForEndOfFrame();
        }

        tlScript.isTeleporting = false;

    }

    IEnumerator goToNextScene(float startAlpha, float endAlpha, string s) {
